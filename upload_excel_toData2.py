...
def open_excel():
    try:
        book=xlrd.open_workbook(r'E:\Api_Auto_Test\Data analysis\city1005.xlsx')
    except:
...
def search_count():
    cursor=db.cursor()
    select="select count(*) from zhang_city"
...
def insert_data():
    ...
    for i in range(1,sheet.nrows): #第一行是标题名，对应表中的字段应该是从第二行开始；
        ...
        print("第",i,"行的值：",value)
        sql="INSERT INTO world.zhang_city(ID,Name,CountryCode,District,Population)VALUES(%s,%s,%s,%s,%s)"
